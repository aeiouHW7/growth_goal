...
| **术语表实时更新** | 发现新术语时立即追加到 `docs/wiki/glossary.md`，不积压到最后 |
...
2. **Step 1 — 定设计方向**：根据产品类型/目标用户/品牌基调，用内置决策表确定风格、配色、字体
...
写入 `planning/{version}/{name}_prd.md` + `planning/{version}/{name}_prototype.html`。
