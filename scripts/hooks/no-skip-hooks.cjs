...
const COMMIT_OPTIONS_WITH_VALUE = new Set([
  ...
  '-c', '--reedit-message', '--author', '--date', '--template',
...
function tokenizeShellWords(input, start = 0, end = input.length) {
  ...
  for (let i = start; i < end; i++) {
    ...
    if (escaped) { beginToken(i - 1); value += char; escaped = false; continue; }
    if (quote) {
      if (char === quote) { quote = null; continue; }
      if (quote === '"' && char === '\\') { beginToken(i); escaped = true; continue; }
      beginToken(i); value += char; continue;
    }
    if (char === '"' || char === "'") { beginToken(i); quote = char; continue; }
    if (char === '\\') { beginToken(i); escaped = true; continue; }
    if (/\s/.test(char)) { pushToken(i); continue; }
    beginToken(i); value += char;
  }
...
function detectGitCommand(input, start = 0) {
  while (start < input.length) {
    ...
    if (bestCmd) {
      return {
        command: bestCmd, offset: bestIdx + bestCmd.length,
        gitStart: git.idx, gitEnd: git.idx + git.len, commandStart: bestIdx,
      };
    }
...
function checkCommand(input) {
  ...
  while (start < input.length) {
    ...
    if (hasHooksPathOverride(input, detected)) {
      return { blocked: true, reason: `BLOCKED: Overriding core.hooksPath is not allowed with git ${gitCommand}. Git hooks must not be bypassed.` };
    }
    if (hasNoVerifyFlag(input, gitCommand, offset)) {
      return { blocked: true, reason: `BLOCKED: --no-verify flag is not allowed with git ${gitCommand}. Git hooks must not be bypassed.` };
    }
